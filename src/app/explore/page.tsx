...
const mockReports: Report[] = [
  ...
  const sectors: string[] = ['Government', 'Law Enforcement', 'Healthcare', 'Education', 'Judicial', 'Private Sector'];
  const locations: string[] = ['Dhaka', 'Chittagong', 'Sylhet', 'Rajshahi', 'Khulna', 'Barishal', 'Rangpur', 'Mymensingh'];
  const dateRanges: string[] = ['Last 7 days', 'Last 30 days', 'Last 3 months', 'Last 6 months', 'Last year'];
  const amountRanges: string[] = ['Under 5,000 BDT', '5,000-20,000 BDT', '20,000-50,000 BDT', 'Over 50,000 BDT'];
  const statuses: string[] = ['Verified', 'Under Investigation', 'Resolved'];

  // Filter and search logic
  const filteredReports: Report[] = mockReports.filter((report: Report) => {
    const matchesSearch: boolean = report.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         report.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         report.organization.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesSector: boolean = !filters.sector || report.sector === filters.sector;
    const matchesLocation: boolean = !filters.location || report.location.includes(filters.location);
    const matchesStatus: boolean = !filters.status || report.status === filters.status;
    
    return matchesSearch && matchesSector && matchesLocation && matchesStatus;
  });

  // Pagination
  const totalPages: number = Math.ceil(filteredReports.length / reportsPerPage);
  const startIndex: number = (currentPage - 1) * reportsPerPage;
  const currentReports: Report[] = filteredReports.slice(startIndex, startIndex + reportsPerPage);

  const getSeverityColor = (severity: Report['severity']): string => {
    switch (severity) {
  ...
  const getStatusColor = (status: Report['status']): string => {
    switch (status) {
      ...
      case 'Under Investigation': return 'text-yellow-400 bg-yellow-500/20';
  ...
  const stats: Stats = {
    ...
    verifiedReports: mockReports.filter((r: Report) => r.status === 'Verified').length,
    totalAmount: mockReports.reduce((sum: number, r: Report) => sum + r.amount, 0),
    avgAmount: Math.round(mockReports.reduce((sum: number, r: Report) => sum + r.amount, 0) / mockReports.length)
  };

  return (
    <>
      <Head>
        <title>Explore Reports - BribeChain</title>
        <meta name="description" content="Browse verified corruption reports from across Bangladesh. All reports are anonymized and stored securely on blockchain." />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in-up">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleFilterChange('sector', e.target.value)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleFilterChange('dateRange', e.target.value)}
                    className="w-full bg-black/30 border border-white/20 rounded-xl px-4 py-3 text-white focus:border-cyan-400 focus:outline-none transition-colors"
                  >
